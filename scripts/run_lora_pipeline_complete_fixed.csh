...
if ( ! $?PIPELINE_MANAGER_SCRIPT ) then
    setenv PIPELINE_MANAGER_SCRIPT "src/training/automated_lora_pipeline.py"
endif

if ( ! $?TRAINER_SCRIPT ) then
    setenv TRAINER_SCRIPT "src/training/lora_local_news_tuning.py"
endif
...
if ( "$action" == "search" || "$action" == "cv" || "$action" == "final" || "$action" == "test" || "$action" == "rank_search" || "$action" == "rank_cv" || "$action" == "report" ) then
    ...
    # Use the Python environment activated before submission.
    if ( ! $?HF_TOKEN ) then
    ...
    if ( ! $?HF_HOME ) then
        setenv HF_HOME "$work_dir/.cache/huggingface"
    endif
